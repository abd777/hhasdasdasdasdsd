  ...
  <v-row justify="end">
    <v-btn color="primary" class="mr-20" @click="dialog=true">Add New</v-btn>
     ...
     <!-- <services :headers="headers" :contents="services" title="Services" :isNew="true" :isIcons="true" :icons="['mdi-pencil','mdi-delete']" :isOrders="false" @editService="editItem($event)" @delete="deleteItem($event)" @addService="add()" /><br/>
        ...
        <v-row  align="center">
          <v-chip :ripple="false" class="ma-2" v-for="(v,k) in 20" :key="k">Service {{ v%2?'sdasasdasdad':'ss' }}</v-chip>
        </v-row>
    <v-dialog v-model="dialog" scrollable max-width="300px">
      <v-card>
        ...
        <v-card-text style="height: 300px;">
                <v-checkbox v-model="selectedServices" :label="'label'+v" :value="'value'+v" v-for="(v,k) in 10" :key="k"></v-checkbox>
        </v-card-text>
        <v-divider></v-divider>
        <v-card-actions>
          <v-btn color="blue darken-1" text @click="dialog = false">Close</v-btn>
          <v-btn color="blue darken-1" class="ml-auto" text @click="dialog = false">Save</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
